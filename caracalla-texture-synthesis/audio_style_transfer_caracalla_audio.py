# optimize generated audio directly rather than spectrogram
...
def extract_log_spectrogram(wav):
    spec = torchaudio.transforms.Spectrogram(n_fft=FRAME_SIZE, hop_length=HOP_SIZE, win_length=FRAME_SIZE).to(device)
    # X = torch.stft(wav, n_fft=FRAME_SIZE, hop_length=HOP_SIZE, win_length=FRAME_SIZE, return_complex=True, window=torch.hann_window(window_length=FRAME_SIZE, device=device))
    X = spec(wav)
    Y = torch.abs(X)
    S = torch.log(1 + COMPRESSION_FACTOR * Y) / np.log(1 + COMPRESSION_FACTOR)
...
#     griffin_lim = torchaudio.transforms.GriffinLim(n_fft=FRAME_SIZE, hop_length=HOP_SIZE, win_length=FRAME_SIZE).to(device)
#     S = griffin_lim(S)
...
content_tensor = extract_log_spectrogram(content_audio)
style_tensor = extract_log_spectrogram(style_audio)
generated_audio = content_audio.clone().requires_grad_(True)

model = Model().to(device).eval()

# out = log_spec_to_wav(content_tensor)
# sf.write("test.wav", out.squeeze(0).numpy(force=True), sr)
...
for step in range(total_steps):
    generated_tensor = extract_log_spectrogram(generated_audio)
    ...
    # G.div(channels * height * width)
    # A.div(channels * height * width)

    style_loss += torch.mean((G - A) ** 2) / torch.mean(A ** 2)
